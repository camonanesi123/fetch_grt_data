...
contract WexMaster is Ownable {
    ...
    function updatePool(uint256 _pid) public {
        ...
        //上一次 池子状态改变 到 本次状态改变 池子里面 新产出的WEX 
        uint256 wexReward = multiplier
            .mul(wexPerBlock)
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        wex.mint(address(this), wexReward);
        // 把赚的WEX 平摊到 每一股待领取收益里面
    ...
    function deposit(uint256 _pid, uint256 _amount, bool _withdrawRewards) public {
        ...
        //统计上一次状态改变 到本次状态改变 单个LP 累计奖励
        updatePool(_pid);
        //如果用户之前再池子里面质押过LP 把之前产生的收益 计算到用户头上
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                //上一次统计区块奖金时候用户累计奖金+ 上一次统计到现在累计的奖金
        ...
        //这里要注意，之前的奖励前面已经兑付过。下次算奖励的时候。这部分算负债。因为单个LP对应的WEX越来越多
